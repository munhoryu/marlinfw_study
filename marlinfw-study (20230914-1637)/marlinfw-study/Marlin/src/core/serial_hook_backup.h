...
template < class Serial0T, const uint8_t offset = 0, const uint8_t step = 1 >
struct MultiSerial : public SerialBase< MultiSerial< Serial0T, offset, step > > {
    typedef SerialBase< MultiSerial< Serial0T, offset, step > > BaseClassT;

    SerialMask portMask;
    Serial0T serial0;
    static constexpr uint8_t Usage = _BV(step) - 1; // A bit mask containing 'step' bits
    static constexpr uint8_t output[] = { Usage << (offset + (step * 1)) };
    static constexpr uint8_t ALL = 0 | output[0];

    NO_INLINE void write(uint8_t c) {
        if (portMask.enabled(output[0])) serial0.write(c);
    }
    NO_INLINE void msgDone() {
        if (portMask.enabled(output[0])) serial0.msgDone();
    }
    int available(serial_index_t index) {
        uint8_t pos = offset;
        if (index.within(pos, pos + step - 1)) return serial0.available(index); else pos += step;
        return false;
    }
    int read(serial_index_t index) {
        uint8_t pos = offset;
        if (index.within(pos, pos + step - 1)) return serial0.read(index); else pos += step;
        return -1;
    }
    void begin(const long br) {
        if (portMask.enabled(output[0])) serial0.begin(br);
    }
    void end() {
        if (portMask.enabled(output[0])) serial0.end();
    }
    bool connected() {
        bool ret = true;
        if (portMask.enabled(output[0]) && !CALL_IF_EXISTS(bool, &serial0, connected)) ret = false;
        return ret;
    }
    ...
    // Redirect flush
    NO_INLINE void flush() {
        if (portMask.enabled(output[0])) serial0.flush();
    }
    NO_INLINE void flushTX() {
        if (portMask.enabled(output[0])) CALL_IF_EXISTS(void, &serial0, flushTX);
    }

    // Forward feature queries
    SerialFeature features(serial_index_t index) const {
        uint8_t pos = offset;
        if (index.within(pos, pos + step - 1)) return serial0.features(index); else pos += step;
        return SerialFeature::None;
    }

    MultiSerial(Serial0T& serial0, const SerialMask mask = ALL, const bool e = false)
        : BaseClassT(e), portMask(mask), serial0(serial0) {}
};
// Build the actual serial object depending on current configuration
#define Serial1Class TERN(SERIAL_RUNTIME_HOOK, RuntimeSerial, BaseSerial)
#define ForwardSerial1Class TERN(SERIAL_RUNTIME_HOOK, RuntimeSerial, ForwardSerial)
