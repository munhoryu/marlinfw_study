...
namespace Private {
    ...
    template <typename T, typename ... Args>  typename enable_if<HasMember_flushTX <T>::value, void>::type Call_flushTX(T* t, Args... a) {
        return static_cast<void>(t->flushTX(a...));
    } 
        //__attribute__((unused)) static void Call_flushTX(...) {
        static void Call_flushTX(...) {
        return;
    }
};
//CALL_IF_EXISTS_IMPL(bool, connected, true);
namespace Private {
    template <typename Type, typename Yes = char, typename No = long> struct HasMember_connected {
        template <typename C> static Yes& test(decltype(&C::connected)); template <typename C> static No& test(...); enum {
            value = sizeof(test<Type>(0)) == sizeof(Yes)
        };
    };
} namespace Private {
    template <typename T, typename ... Args>  typename enable_if<HasMember_connected <T>::value, bool>::type Call_connected(T* t, Args... a) {
        return static_cast<bool>(t->connected(a...));
    } 
        //__attribute__((unused)) static bool Call_connected(...) {
        static bool Call_connected(...) {
        return true;
    }
};
//CALL_IF_EXISTS_IMPL(SerialFeature, features, SerialFeature::None);
namespace Private {
    template <typename Type, typename Yes = char, typename No = long> struct HasMember_features {
        template <typename C> static Yes& test(decltype(&C::features)); template <typename C> static No& test(...); enum {
            value = sizeof(test<Type>(0)) == sizeof(Yes)
        };
    };
} namespace Private {
    template <typename T, typename ... Args>  typename enable_if<HasMember_features <T>::value, SerialFeature>::type Call_features(T* t, Args... a) {
        return static_cast<SerialFeature>(t->features(a...));
    } 
        //__attribute__((unused)) static SerialFeature Call_features(...) {
        static SerialFeature Call_features(...) {
